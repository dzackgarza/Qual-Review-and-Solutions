...
Consider SL2(R) acting on \( {\mathbb{R}}^2 \) by matrix multiplication. What is the stabiliser of a point? Does it depend which point? Do you know what sort of subgroup this is? What if SL2(R) acts by Möbius transformations instead?
...
Talk about the isomorphism classes of subgroups of Q. How many are there? Are the ones you've given involving denominators divisible only by certain primes distinct? So that gives you the cardinality. Are these all of them?
...
Talk about the possible subgroups of \( Z\times Z \times Z \). Now suppose that you have a subgroup of \( Z \times Z \times Z \). What theorem tells you something about the structure of the quotient group?
...
Suppose I asked you to classify groups of order 4. Why isn't there anything else? Which of those could be realised as a galois group over Q?
...
How would you start finding the groups of order 56? Is there in fact a way for Z/7Z to act on a group of order 8 nontrivially?
...
Take a finite field extension Fpn over Fp. What is Frobenius? What is its characteristic polynomial?
...
What is a separable extension? Can Q have a non-separable extension? How about Z/pZ? Why not? Are all extensions of characteristic 0 fields separable? Of finite fields? Prove it.
...
What is galois theory? State the main theorem. What is the splitting field of \( x^5 - 2 \) over Q? What are the intermediate extensions? Which extensions are normal, which are not, and why? What are the galois groups (over Q) of all intermediate extensions?
...
Is abelian galois extension transitive? That is, if K has abelian galois group over E, E has abelian galois group over F , and K is a galois extension of F , is it necessarily true that gal(K/F ) is also abelian? give a counterexample involving number fields as well as one involving function fields.
...
Is Q(21/3) normal? What is its splitting field? What is its galois group? Draw the lattice of subfields.

### Question 3.20

What's the galois group of \( x^2 + 1 \) over Q? What's the integral closure of Z in Q(i)?
...
What is the galois group of `\todo[inline]{Missing}`{=tex} over Q?
...
How would you find the galois group of \( x^3 + 2x + 1 \)? Adjoin a root to Q. Can you say something about the roots of \( x^3 + 3x + 1 \) in this extension?
...
How do you construct a polynomial over Q whose galois group is \( S_n \)? Do it for n = 7 in particular.
...
Suppose you are given a finite galois extension K/Q by \( f (x) \in {\mathbb{Z}}[x] \) such that deg(f ) = n and gal(K/Q) = \( S_n \). What can you say about the roots?

### Question 3.55

How many automorphisms does the complex field have? How can you extend 2) of an algebraic field into C? How can you extend a simple automorphism (e.g. a general subfield isomorphism? What feature of C allows you to? `\todo[inline]{Missing}`{=tex}
...
Consider the minimal polynomial f (x) for a primitive mth root of unity. Prove that if p divides f (a) for some integer a and (p, m) = 1 then m divides p - 1. Use this fact to show that there are infinitely many primes congruent to 1 (mod m).
...
Can you have a degree 7 irreducible polynomial over Fp? How about a degree 14 irreducible polynomial?
...
Can we solve general quadratic equations by radicals? \( A_n \)d what about cubic and so on? Why can't you solve 5th degree equations by radicals?

### Question 3.64

Talk about solvability by radicals. Why is \( S_5 \) not solvable? Why is A5 simple?
...
State and prove Eisenstein's Criterion. \#\#\# Question 3.69 Why is (xp - 1)/(x - 1) irreducible over Q?
...
Say we can find a matrix A such that exp(A) = B for B in SLn(R). Does A also have to be in SLn(R)? Can you take A to be in SLn(R)?
...
Is C\[x, y\] a PID? Is (cid:104)x, y(cid:105) a prime ideal in it?
...
Factor 6 in two different ways in \( Z[\sqrt{-5}] \) Is there any way to explain the two factorisations? Factor the ideal generated by 6 into prime ideals.
...
Let O be the ring of integers of Q(d), where d \> 0. What can you say about the quotient of O by one of its prime ideals?
...
Talk about factorisation and primes in a polynomial ring. What is irreducibility? For what rings R is it true that R\[\$x\^1, · · · , xn\] is a unique factorisation domain? What is wrong with unique factorisation if we don't have a domain? Now, PIDs are Noetherian, but are there UFDs which are not?
...
Define "radical". Prove it is an ideal. Prove that the ideal of all polynomials vanishing on the zero set of I is rad(I).
...
Is the map `\todo[inline]{Missing}`{=tex} an isomorphism?

### Question 6.30

Describe the left, right, and two-sided ideals in the ring of square matrices of a fixed size. Now identify the matrix algebra Mn(K) with \( EndK(V ) \) where V is an \( n \)- dimensional K-vector space. Try to geometrically describe the simple left ideals and also the simple right ideals via that identification.
...
What if we take the noncyclic group of order 4 instead of Z/5Z? The quaternion group instead of Z/5Z?
...
If M is free abelian, how can I put quotients of M in some standard form? What was crucial about the integers here (abelian groups being modules over Z)? How does the procedure simplify if the ring is a Euclidean domain, not just a PID?
...
Now we'll take the tensor product of two abelian groups, that is, Z-modules. Take \( {\mathbb{Z}}/p{\mathbb{Z}} \) and \( {\mathbb{Z}}/q{\mathbb{Z}} \), where \( p \) and \( q \) are distinct primes. What is their tensor product?
...
If you have a Z/5Z action on a complex vector space, what does this action look like? What about an \( S_3 \) action? A dihedral group of any order?
