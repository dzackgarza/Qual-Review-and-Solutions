...
- $G(x^4-2) = D_4$: 
  - The roots are $\zeta_2^k \omega$ for $0\leq k \leq 3$, where $\zeta_2 = i, \omega = 2^{1\over 4}$.
    So the splitting field is $\QQ(\omega, \zeta_2)$.
    This has degree $4\phi(2) = 8$, and $D_4 \leq S_4$ is the only transitive subgroup of order 8.
...
- $G(x^4+2) = D_4$: 
  ...
  the splitting field is $\QQ(2^{1\over 4}, i)$ which has degree $4\cdot 2=8$
