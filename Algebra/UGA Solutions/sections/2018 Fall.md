...
we can bound strictly bound the size by ignoring the fact that the conjugates may intersect, and also overcounting their intersections at the identity:
\begin{align*}
...
& = [G: N_G(H)] \abs{H} \\
&= {\abs{G} \over \abs{N_G(H)}} \abs{H} \qtext{since the groups are finite}
.\end{align*}
