...
- Burnside's Lemma: $\abs{X/G} \abs{G} = \sum_{g\in G} \abs{X^g}$ (number of orbits equals average number of fixed points).
...
,\end{align*}
we can *strictly* bound the size of the union of conjugates by ignoring the fact that they may intersect, and also overcounting their intersections at the identity:
\begin{align*}
