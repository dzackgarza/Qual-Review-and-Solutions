...
- If $G = P$, then the result follows immediately, so assume $P<G$ is proper.
- This also rules out $M \subseteq P$ by maximality of $M$, so $P\intersect M = \theset{\id}$.
...
&\implies \abs{G} = \abs{M} \abs{P} \quad\quad\text{since }M\intersect P = \emptyset \\ \\
&\implies \abs{G} / \abs{M} \text{ divides } \abs{P} = p^c
.\end{align*}
