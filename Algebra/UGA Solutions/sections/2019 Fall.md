...
We can now note that $\inner{\vector e_k}{\vector v} = \sum_{j=1}^n v_j \vector e_k \cdot \vector e_j = 0$ for every $k$, so $\vector w$ is orthogonal to every $\vector e_k$, and thus its span.
