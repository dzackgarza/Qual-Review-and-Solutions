...
\textbf{Showing a function is constant:}
...
\textbf{Showing a function is zero}: Show \(f\) is entire, bounded, and
...
\begin{definition}[Pole Terminology]
...
\begin{theorem}[Holomorphic functions have harmonic components]

If \(f(z) = u(x, y) + iv(x, y)\), then \(u, v\) are harmonic.

\end{theorem}

\begin{theorem}[Holomorphic functions are continuous.]
...
\begin{proposition}[For simple poles]
...
Example: Let \(f(z) = \frac{1}{1+z^2}\), then
