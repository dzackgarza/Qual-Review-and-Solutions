...
\(\varepsilon\) in continuity, but only depends on \(\varepsilon\) in
the uniform version. I.e. once \(\delta\) is fixed, for continuity one
may only range over \(x\), but in uniform continuity one can range over
all pairs \(x,y\).
...
\begin{theorem}[Cauchy Remainder]
...
\hypertarget{the-dirichlet-function}{%
\subsubsection{The Dirichlet function}\label{the-dirichlet-function}}
...
\(L^1 \subset (L^\infty) {}^{ \check{} }\), since the isometric mapping
is always injective, but \emph{never} surjective.
...
\((H {}^{ \check{} }, {\left\lVert {{-}} \right\rVert}_{\text{op}})\) is
a normed space.
...
\((X {}^{ \check{} }, {\left\lVert {{-}} \right\rVert}_{\text{op}})\) is
a Banach space.
...
\item
  Since \(\left\{{L_{n}}\right\}\) is Cauchy in \(X {}^{ \check{} }\),
...
\item
  ...
  f &\mapsto \int_0^1 fg
  \end{align*}
  defines an element of \(L^1([0, 1]) {}^{ \check{} }\) with
  \({\left\lVert {\Lambda_g} \right\rVert}_{L^1([0, 1]) {}^{ \check{} }}= {\left\lVert {g} \right\rVert}_{L^\infty([0, 1])}\).
\end{enumerate}
...
\item
  Prove that
  \((X {}^{ \check{} }, {\left\lVert {{-}} \right\rVert}_{^{\operatorname{op}}})\)
  is a Banach space.
\end{enumerate}
