...
- Claim: **$K$ is compact**.
  ...
  - Strategy: write $K^c$ as the union of open intervals (since these form a basis for the Euclidean topology on $\RR$).

  - Identify $K^c$ as the set of real numbers in $[0, 1]$ whose decimal expansion **does** use a 4.
  ...
  - Set $r < 10^{-k}$ and pick any $y \in [0, 1]$ such that $y\in B_r(x)$, i.e. $\abs{x-y} < 10^{-k}$. 
