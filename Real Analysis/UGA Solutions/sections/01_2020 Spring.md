...
Finally, the $\int_0^1 kx^{k-1} p_\eps(x) \,dx = p_\eps(1)$ for each $\eps$ by the first argument, and $p_\eps(1) \converges{\eps\to 0}\to f(1)$ since uniform convergence implies pointwise convergence.
...
> Proof of theorem: if $Q$ is a closed cube, then $m_*(Q) = \abs{Q}$, the usual volume.
>
> - Since $Q\subseteq Q$, $Q\covers Q$ and $m_*(Q) \leq \abs{Q}$ since $m_*$ is an infimum over such coverings.
> - Other direction: show $\abs{Q} \leq m_*(Q) + \eps$ for every $\eps> 0$.
> - Let $\theset{Q_i} \covers Q$ be arbitrary, it suffices to show that $$\abs{Q} \leq \qty{\sum \abs{Q_i}} + \eps.$$
> - Pick open cubes $S_i$ such that $\abs{Q_i} \leq \abs{S_i} \leq (1+\eps)\abs{Q_i}$.
> - Then $\theset{S_i} \covers Q$, so by compactness pick a finite subcover of size $N$
> - Conclude with 
\begin{align*}
Q \subseteq \union_{i=1}^N S_i \implies \abs{Q} \leq \sum_{i=1}^N \abs{S_i} \leq \sum_{i=1}^N (1+\eps) \abs{Q_j} \leq (1+\eps)\sum_{i=1}^\infty \abs{Q_i } \converges{\eps\to0}\to \sum_{i=1}^\infty \abs{Q_i}
.\end{align*}
> - (Or note that taking an infimum over coverings on the RHS preserves the inequality.)
