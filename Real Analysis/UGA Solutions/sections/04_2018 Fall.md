...
- Claim: $E$ is measurable $\implies$ there exists an open $O\supseteq E$ such that $m(O\setminus E) = 0$.
  ...
  - Suppose $E$ is bounded.
    ...
    \begin{align*}
    m_*(O\setminus E) = \lim_{N\to\infty} m_*(O_N\setminus E) \leq \lim_{N\to \infty} m_*(G_N\setminus E) = \lim_{N\to\infty} {1\over N} = 0
    ,\end{align*}
...
- Since $E^c$ is measurable exists an open $O\supseteq E^c$ such that $m(O\setminus E_c) = 0$.
