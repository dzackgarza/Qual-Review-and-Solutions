...
- Set $$E\definedas \theset{(x, t) \in \RR^{n} \cross \RR \suchthat f(x) > t}.$$
- Note that we can rewrite $E = \theset{(x, t) \in \RR^n \cross \RR \suchthat 0 \leq t < f(x)}$ since $f\geq 0$.
- Note that we have slices 
\begin{align*}
...
.\end{align*}
  - Note that the RHS of the first is precisely what appears in the original RHS integrand.o
  - Note that $E^x = [0, f(x)] \subset \RR$ and thus $m(E^x) = f(x)$.
- Since $f$ is measurable, $E$ is measurable and thus $m(E)$ makes sense.
- Claim: for $E$ defined above, $m(E) = \int f$.
\begin{align*}
m(E) 
&= \int_{\RR^{n+1}} \chi_E(x, t) \\
&= \int_{\RR^n} \int_\RR \chi_E(x, t) \,dx \,dt \quad{by Tonelli}\\
&= \int_{\RR^n} m(E^x) \,dx \\
&= \int_{\RR^n} f(x) \,dx 
.\end{align*}
- On the other hand,
\begin{align*}
m(E) 
&= \int_{\RR^{n+1}} \chi_E(x, t) \,dx \,dt \\
&= \int_\RR \int_{\RR^n} \chi_E(x, t) \, dx \,dt
.\end{align*}
