...
- We can rewrite $E = \theset{(x, t) \in \RR^n \cross \RR \suchthat 0 \leq t < f(x)}$ since $f\geq 0$.
- We have slices 
\begin{align*}
...
.\end{align*}
  - Note that $E_t$ precisely the set that appears in the original RHS integrand.
...
m(E) 
&= \int_{\RR^{n+1}} \chi_E(x, t) \\
&= \int_\RR \int_{\RR^n} \chi_E(x, t) \, dx \,dt \quad\text{Tonelli} \\
&= \int_\RR m(E^t) \,dt
.\end{align*}
