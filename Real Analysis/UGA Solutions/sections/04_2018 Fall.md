...
- Set $$E\definedas \theset{(x, t) \in \RR^{n} \cross \RR \suchthat f(x) \geq y}.$$
...
- By the claim,
\begin{align*}
m(E) = \int_\RR m(E^t) \,dt \definedas \int_{-\infty}^\infty m\qty{\theset{ x\suchthat f(x) \geq t}} = \int_0^\infty m\qty{\theset{x\suchthat f(x) \geq t}} \quad\text{since } f\geq 0
.\end{align*}
