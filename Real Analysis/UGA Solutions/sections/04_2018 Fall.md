...
.\end{align*}
  - $E_t$ precisely the set that appears in the original RHS integrand.
  - $m(E^x) = f(x)$.
...
- Claim: If $E$ is measurable on a product space $\RR^a \cross \RR^b \ni (x, y)$, then for almost every $y$, $E^y$ is a measurable set, $y\mapsto m(E^y)$ is a measurable function, and $m(E) = \int_\RR m(E^y) \, dy$.
- On one hand, 
\begin{align*}
m(E) 
