...
- Take the sequence $\varepsilon_n = \frac 1 n \to 0$ to produce a sequence of closed sets $F_n$ such that $$m(E\setminus F_n) < \frac 1 n \qtext{for all} n.$$
- Let $F \definedas \union_{n=1}^\infty F_n$, which is a union of closed sets and thus $F_\sigma$, and thus a Borel.
