...
- Claim: since $E$ is measurable, $E^c$ is measurable
  ...
  \begin{align*}
  E^c\setminus S 
  &\definedas E^c \setminus \union_{n=1}^\infty \OO_n^c \\
  &\definedas E^c \intersect \qty{\intersect_{n=1}^\infty \OO_n}^c  \quad \text{also using De Morgan's law}\\
  ...
  .\end{align*}
  - Then $m_*(E^c\setminus S) \leq m_*(\OO_N \setminus E) \leq {1\over N}$ for every $N$, forcing $m_*(E^c\setminus S) = 0$.
  - So $E^c\setminus S$ is measurable as desired.
...
$$
 where the convergence occurs because $\cos(tx)$ is differentiable in $t$.
