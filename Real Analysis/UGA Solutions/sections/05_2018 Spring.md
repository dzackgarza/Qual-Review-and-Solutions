...
- $\hat f \equiv 0$ iff $\inner{f}{e_k} \definedas \int_X f(x) e_k(x) \,dx =0$ for every $k$, where $e_k$ is any basis for $L^2([0, 1])$ (such as $e_k \definedas \exp{2\pi i k}$).
...
- But then the $k$th Fourier coefficient $a_k$ of $f$ is given by
\begin{align*}
a_k \definedas \hat f(k) \definedas \inner{f}{e_k} 
