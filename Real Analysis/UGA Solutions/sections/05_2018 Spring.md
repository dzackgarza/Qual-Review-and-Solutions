...
\end{align*}
  since an element $x$ is in the LHS $\iff x \in E_j$ for infinitely many $j \iff$ there are infinitely many $j$ for which there exist a $p$ such that $\abs{x - {p\over j}} < j^{-3} \iff$ there are infinitely many such pairs $p, j$ $\iff x\in E$.
