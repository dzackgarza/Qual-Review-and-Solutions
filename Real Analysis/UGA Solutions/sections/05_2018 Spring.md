...
- Since $f\in L^1$, $\int 2\abs{f} < \infty$ and it makes sense to subtract it from both sides, thus
\begin{align*}
...
.\end{align*}
  which forces $\limsup_n \int \abs{f_n -f}  = 0$, since
    ...
    - The integral of a positive function is positive, so $\int \abs{f_n - f} \geq 0$.
    - So $-\int \abs{f_n - f} \leq 0$.
    - But the above inequality shows $-\int \abs{f_n - f} \geq 0$ as well.
...
\leq \int \abs{f_n - f}
,\end{align*}
  and taking $\limsup_n$ on both sides yields
  \begin{align*}
  \limsup_n \abs{\int f_n - \int f} \leq \limsup_n \int \abs{f_n - f} = 0
  ,\end{align*}
  so $\int f_n \converges{n\to\infty}\to \int f$.
