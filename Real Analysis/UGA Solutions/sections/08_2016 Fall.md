...
- Claim: $\sum n^{-x}\ln(n)$ converges.
  ...
  ,\]
  - This implies that for a fixed $\eps >0$ and for any constant $c>0$ there exists an $N$ large enough such that $n\geq N$ implies $\ln(n)/n^\delta < \eps$, i.e. $\ln(n) < \eps n^{\delta}$.
